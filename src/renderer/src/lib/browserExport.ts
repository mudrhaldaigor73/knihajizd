...
export function downloadCsv(data: LogbookData, filename = `kniha-jizd-${new Date().toISOString().slice(0, 10)}.csv`) {
  ...
  downloadBlob(new Blob([[header.map(escape).join(";"), ...rows].join("\n")], { type: "text/csv;charset=utf-8" }), filename);
}
...
export async function downloadXlsx(data: LogbookData, filename = `kniha-jizd-export-${new Date().toISOString().slice(0, 10)}.xlsx`) {
  ...
  tripsSheet.addRow({ purpose: "Celkem", km: { formula: `SUM(M2:M${Math.max(2, tripsSheet.rowCount)})` } }).font = { bold: true };
  ...
  fuelSheet.addRow({ station: "Celkem", totalPrice: { formula: `SUM(G2:G${Math.max(2, fuelSheet.rowCount)})` } }).font = { bold: true };
