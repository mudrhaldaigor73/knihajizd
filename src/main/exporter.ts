...
import type { FuelRecord, LogbookData, Trip, Vehicle } from "../shared/types.js";
...
export async function exportXlsx(filePath: string, data: LogbookData) {
  ...
  const totalRow = tripsSheet.addRow({
    purpose: "Celkem",
    km: { formula: `SUM(M2:M${Math.max(2, tripsSheet.rowCount - 1)})` }
  });
  ...
  const fuelTotalRow = fuelSheet.addRow({
    station: "Celkem",
    totalPrice: { formula: `SUM(G2:G${Math.max(2, fuelSheet.rowCount - 1)})` }
  });
...
export async function exportCsv(filePath: string, data: LogbookData) {
  ...
  await fs.writeFile(filePath, [header.map(escape).join(";"), ...rows].join("\n"), "utf8");
}
