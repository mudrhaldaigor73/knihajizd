...
import { monthlySummaries, yearlySummaries } from "./summaries.js";
...
export function buildWorkbook(data: LogbookData): ExcelJS.Workbook {
  ...
  monthSheet.columns = [
    { header: "Měsíc", key: "period", width: 12 },
  ...
  monthlySummaries(data).forEach((row) => monthSheet.addRow(row));
  styleSheet(monthSheet);

  const yearSheet = workbook.addWorksheet("Roční souhrn");
  yearSheet.columns = [
    { header: "Rok", key: "period", width: 10 },
    { header: "Celkem km", key: "km", width: 14 },
    { header: "Služební km", key: "business", width: 15 },
    { header: "Soukromé km", key: "private", width: 15 },
    { header: "Náklady tankování", key: "fuel", width: 18 },
  ...
  yearlySummaries(data).forEach((row) => yearSheet.addRow({ ...row, createdAt: new Date().toLocaleString("cs-CZ") }));
